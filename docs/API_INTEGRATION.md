...
│   ├── helpers/               # Business logic wrappers (EDIT THIS)
│   │   ├── findings.go        # Findings endpoints (SAST/DAST) with filtering
...
✅ **Regenerating client**: Only affects `go-client-generated/`, api wrapper shields tools  
✅ **Testing**: Mock api package, not generated client  

## Usage Pattern

### Step 1: Define API Method (api/helpers/)

```go
// api/helpers/findings.go
package api
...
Only create a helper in `api/helpers/` if you need type conversion, filtering, or business logic:
...
### 5. Create Helper (if needed)

Only create a helper in `api/` if you need type conversion, filtering, or business logic:

For simple APIs, skip the helper and call the generated client directly from tools.
...
1. ✅ **Structure created**: api/ package with client, helpers/, generated/
2. ✅ **4 API clients integrated**: healthcheck, findings, dynamic_flaw, static_finding_data_path
3. ✅ **Helpers created**: findings.go with filtering/conversion, health.go
...
- **API helpers**: `api/helpers/` (business logic, filtering, type conversion)
- **Tool usage**: `tools/` package (imports api, uses helpers or client directly)

**Many:many relationships handled by:**
- API helpers shared across multiple tools
- Tools can call multiple API helpers
- Clean separation via 3-layer architecture: tools → helpers → generated clients
